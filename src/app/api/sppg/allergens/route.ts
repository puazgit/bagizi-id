 ...
 * - POST: Protected by withSppgAuth (requires sppgId)
 * - Automatic audit logging for all operations
 * - Multi-tenant isolation: Platform allergens + SPPG custom allergens
 */
 ...
 * 
 * @rbac Protected by withSppgAuth - requires sppgId
 * @audit Automatic logging via middleware
 */
export async function POST(request: NextRequest) {
  return withSppgAuth(request, async (session) => {
    try {
      ...
      // 3. Check if allergen name already exists for this SPPG
