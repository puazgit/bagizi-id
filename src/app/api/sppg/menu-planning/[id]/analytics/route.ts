...
export async function GET(
  ...
  try {
      ...
      include: {
        program: {
          select: {
            ...
            targetGroup: true // Fixed: ageGroup doesn't exist, use targetGroup
          }
        },
        assignments: {
          include: {
    ...
    // 5. Calculate nutrition analytics
    ...
    plan.assignments.forEach(assignment => {
      ...
      const nutrition = menu.nutritionCalc // Access nutrition calculation
    ...
    plan.assignments.forEach(assignment => {
      assignment.menu.ingredients?.forEach(ing => {
        if (ing.inventoryItemId) { // Check for null
          allIngredients.add(ing.inventoryItemId)
        }
      })
    })

    // 8. Generate compliance checks
    const dailyNutritionCompliance = Object.values(nutritionByDay).map(day => {
      // Sample compliance thresholds (should be configurable per age group)
    ...
    return Response.json({
      ...
      data: {
        planId: plan.id,
        planName: plan.name, // Fixed: field is 'name' not 'planName'
        dateRange: {
        ...
        program: {
          ...
          targetBeneficiaries: plan.program.targetRecipients, // Fixed: correct field
          targetGroup: plan.program.targetGroup // Fixed: use targetGroup not ageGroup
        },
        nutrition: {
